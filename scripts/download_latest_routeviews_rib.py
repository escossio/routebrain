...
from app.collectors.routeviews_downloader import (  # noqa: E402
    RouteViewsDownloadError,
    download_latest_rib,
...
def main() -> None:
    ...
    try:
        result = download_latest_rib()
    except RouteViewsDownloadError as exc:
        console.print(Panel.fit(str(exc), title="RouteViews RIB download", style="red"))
    ...
    manifest_path = processed_dir / f"routeviews_latest_rib_{collector}.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    table = Table(title="RouteViews latest RIB")
